...
description: >
  ...
  the tracetool-mcp-server. Use when the user asks to log, trace, debug, or send
...
- Override via env variable: `TRACETOOL_HOST=127.0.0.1:85`

## MCP server setup

Add to Claude Code's MCP configuration (`~/.claude/mcp.json` or project `.claude/mcp.json`):
...
```json
{
  "mcpServers": {
    "tracetool": {
      "command": "node",
      "args": ["C:/Thierry/Tracetool/NodeJs/mcp-server/server.js"],
      "env": {
        "TRACETOOL_HOST": "127.0.0.1:81"
      }
    }
  }
}
