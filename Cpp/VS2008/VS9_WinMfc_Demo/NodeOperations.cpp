	...
	: CPropertyPage(IDD_NODEEX)    // NodeOperations::IDD
{
   start1 = NULL ;
   start2 = NULL ;
   lastToggleBookmark = false;
   lastToggleVisible = false;
}
...
BEGIN_MESSAGE_MAP(NodeOperations, CPropertyPage)
   ...
   ON_BN_CLICKED(IDC_ToogleBookmark, &NodeOperations::OnBnClickedTooglebookmark)
   ON_BN_CLICKED(IDC_ToogleVisible, &NodeOperations::OnBnClickedTooglevisible)
END_MESSAGE_MAP()
...
void NodeOperations::OnBnClickedTooglebookmark()
{
   if (start2 == NULL)
       return ;
   start2->traceNode->SetBookmark(lastToggleBookmark);
   lastToggleBookmark = !lastToggleBookmark;
}

void NodeOperations::OnBnClickedTooglevisible()
{
   if (start2 == NULL)
       return ;
   start2->traceNode->SetVisible(lastToggleVisible);
