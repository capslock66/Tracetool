// CppWin32Plugin.cpp : Defines the initialization routines for the DLL.
//
...
extern "C"
{
   ...
   __declspec(dllexport) BOOL __cdecl OnAction (char * WinId , int ResourceId ,char *  NodeId) 
   {
      ...
      default : 
         char buffer [50] ;
         sprintf_s(buffer, 50,"On action. Resource Id : %d ", ResourceId); 
   ...
   __declspec(dllexport) void __cdecl OnTimer() 
   {
      ...
      SYSTEMTIME Time;
      GetLocalTime(&Time);
      sprintf_s(buffer, 20, "%02d:%02d:%02d", Time.wHour, Time.wMinute, Time.wSecond); 
