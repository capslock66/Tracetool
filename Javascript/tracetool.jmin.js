"use strict";(function(global){var ttraceScript=null;var headId=null;var request;var stackTrace;var uuid;var requestId=0;var toSend=[];var nbDone=0;var winTraceSingeton=null;var watchesSingeton=null;var clientId="";var host="127.0.0.1:81";var traceClasses={};var isChromeExtension;var isBrowser;var isNodeJs;detectEnvironment();console.log("isChromeExtension : "+isChromeExtension);console.log("isBrowser         : "+isBrowser);console.log("isNodeJs          : "+isNodeJs);if(isNodeJs)
{request=require('request');stackTrace=require('stack-trace');uuid=require('uuid');clientId=uuid().replace(/-/g,'');}else if(isBrowser){ttraceScript=null;headId=global.document.getElementsByTagName("head")[0];}
function detectEnvironment()
{isChromeExtension=false;isBrowser=false;isNodeJs=false;try{if(typeof chrome==="object"&&typeof chrome.extension==="object")
isChromeExtension=true;else if((typeof require==="function")&&(typeof process==="object")&&(process.release.name.search(/node|io.js/)!==-1))
isNodeJs=true;else
isBrowser=true;}
...
xhr.addEventListener("error",function(e3){setTimeout(worker,0);},false);xhr.onload=function(e4){var onloadRequest=e4.currentTarget;var script=onloadRequest.responseText;if(script.startsWith("ttrace.setClientID("))
...
var ttrace={queryClientId:function()
...
flags+=1;commandList.push(intToStr5(100)+intToStr11(flags)+text);sendToClient(commandList);},_done:function(msgId,partNum)
{nbDone++;afterRun();}};Object.defineProperties(ttrace,{"classname":{value:"ttrace",enumerable:true,configurable:false},"host":{get:function(){return host;},set:function(value){host=value;},enumerable:true,configurable:false},"waitingMessageCount":{get:function(){return nbDone;},enumerable:true,configurable:false},"sendMessageCount":{get:function(){return toSend.length;},enumerable:true,configurable:false},"clientId":{get:function(){return clientId;},set:function(value){clientId=value;},enumerable:true,configurable:false},"winTrace":{get:function(){if(winTraceSingeton===null)
...
{if(!this.enabled)
return new traceClasses.TraceNode(this);var result=new traceClasses.TraceNodeEx(this,true);var commandList=prepareNewNode(this,leftMsg||"Object",result.id);result.addObject(objToSend,displayFunctions);result.members.addToStringList(commandList);sendToWinTraceClient(commandList,this.winTraceId);return new traceClasses.TraceNode(result);},sendStack:function(leftMsg,belowFn)
{if(!this.enabled)
return new traceClasses.TraceNode(this);belowFn=belowFn||this.sendStack;var result=new traceClasses.TraceNodeEx(this,true);var commandList=prepareNewNode(this,leftMsg||"Stack",result.id);result.addStackTrace(belowFn);result.members.addToStringList(commandList);sendToWinTraceClient(commandList,this.winTraceId);return new traceClasses.TraceNode(result);},sendCaller:function(leftMsg,belowFn)
{if(!this.enabled)
return new traceClasses.TraceNode(this);belowFn=belowFn||this.sendCaller;var result=new traceClasses.TraceNodeEx(this,true);var commandList=prepareNewNode(this,leftMsg||"Caller",result.id);result.addCaller(belowFn);result.members.addToStringList(commandList);sendToWinTraceClient(commandList,this.winTraceId);return new traceClasses.TraceNode(result);},sendDump:function(leftMsg,shortTitle,buffer,count)
{if(!this.enabled)
...
{that.winTraceId=that.id;debugInstance=new traceClasses.TraceToSend(null,false,contextInstance);debugInstance.iconIndex=24;debugInstance.winTraceId=that.id;debugInstance.enabled=true;warningInstance=new traceClasses.TraceToSend(null,false,contextInstance);warningInstance.iconIndex=22;warningInstance.winTraceId=that.id;warningInstance.enabled=true;errorInstance=new traceClasses.TraceToSend(null,false,contextInstance);errorInstance.iconIndex=23;errorInstance.winTraceId=that.id;errorInstance.enabled=true;};};var winTracePrototype=new traceClasses.TraceToSend();traceClasses.WinTrace.prototype=winTracePrototype;extend(winTracePrototype,{saveToTextFile:function(fileName)
...
classGroup.add(e2,e2.message);}},addCaller:function(belowFn)
{if(!this.enabled)
return;belowFn=belowFn||this.addCaller;var group=new traceClasses.MemberNode("Call stack").setFontDetail(0,true);group.viewerKind=4;this.members.add(group);var stack=stackTrace.get(belowFn);var stackLength=stack.length;for(var i=0;i<stackLength;i++)
{var callObj=stack[i];if(callObj.getFileName().includes("tracetool.js")===false)
{var callName=callObj.toString();group.add(callName);return;}}},addStackTrace:function(belowFn)
{if(!this.enabled)
return;belowFn=belowFn||this.addStackTrace;var group=new traceClasses.MemberNode("Call stack").setFontDetail(0,true);group.viewerKind=4;this.members.add(group);var stack=stackTrace.get(belowFn);var stackLength=stack.length;for(var i=0;i<stackLength;i++)
{var callObj=stack[i];if(callObj.getFileName().includes("tracetool.js")===false)
{var callName=callObj.toString();group.add(callName);}}},send:function()
...
this.members=new Array();}};Object.defineProperty(traceClasses.MemberNode.prototype,'classname',{enumerable:true,configurable:false,writable:false,value:'TraceClasses.MemberNode.prototype'});if(isNodeJs)
module.exports=ttrace;else
