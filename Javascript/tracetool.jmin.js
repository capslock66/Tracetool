"use strict";(function(global){var ttraceScript=null;var headId=null;var request;var stackTrace;var uuid;var requestId=0;var toSend=[];var nbDone=0;var winTraceSingeton=null;var watchesSingeton=null;var clientId="";var host="127.0.0.1:81";var traceClasses={};var isChromeExtension;var isBrowser;var isNodeJs;detectEnvironment();if(isNodeJs)
...
{if(!this.enabled)
return;level=level||0;var group=new traceClasses.MemberNode("Call stack").setFontDetail(0,true);group.viewerKind=4;this.members.add(group);if(isNodeJs)
{var stack=stackTrace.get(this.addCaller);var stackLength=stack.length;for(var i=0;i<stackLength;i++)
{var callObj=stack[i];if(callObj.getFileName().includes("tracetool.js")===false)
{if(level>0)
level--;else{var callName=callObj.toString();group.add(callName);return;}}}}else{var stack=new Error().stack;var stackList=stack.split('\n');var stackLength=stackList.length;for(var i=0;i<stackLength;i++)
{var callObj=stackList[i].trim();if(callObj==="Error")
continue;if(callObj.includes("tracetool.js")===true)
continue;if(callObj.startsWith("at "))
{if(level>0)
level--;else{var callName=callObj.substring(3);group.add(callName);return;}}}}},addStackTrace:function(level)
{if(!this.enabled)
return;level=level||0;var group=new traceClasses.MemberNode("Call stack").setFontDetail(0,true);group.viewerKind=4;this.members.add(group);if(isNodeJs)
{var stack=stackTrace.get(this.addStackTrace);var stackLength=stack.length;for(var i=0;i<stackLength;i++)
{var callObj=stack[i];if(callObj.getFileName().includes("tracetool.js")===false)
{if(level>0)
level--;else{var callName=callObj.toString();group.add(callName);}}}}else{var stack=new Error().stack;var stackList=stack.split('\n');var stackLength=stackList.length;for(var i=0;i<stackLength;i++)
{var callObj=stackList[i].trim();if(callObj==="Error")
continue;if(callObj.includes("tracetool.js")===true)
continue;if(callObj.startsWith("at "))
{if(level>0)
level--;else{var callName=callObj.substring(3);group.add(callName);}}}}},send:function()
