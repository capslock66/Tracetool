...
procedure FillChar2(var Dest; Count: Integer; Value: Byte);
...
function Trim2(const S: RawUTF8): RawUTF8;
...
const
  ...
  // - as generated by DateToSQL/DateTimeToSQL functions, and expected by
...
/// convert an Iso8601 date/time (bit-encoded as Int64) to a ISO-8601 string
// format for SQL '?' inlined parameters
...
begin
  ...
  if IsFixedWidthCodePage(aCodePage) then begin
    ...
    fillchar2(fWideToAnsi[1],65535,ord('?')); // '?' for unknown char
    for i := 1 to 255 do
...
{$endif OWNNORMTOUPPER}
  ...
  fillchar2(ConvertHexToBin[0],sizeof(ConvertHexToBin),255); // all to 255
  v := 0;
...
begin
  ...
  fillchar2(SystemInfo,sizeof(SystemInfo),0);
...
begin
  ...
  fillchar2(ConvertBase64ToBin[0],256,255); // invalid value set to -1
...
begin
  if TimeStamp=0 then
    ...
    SetLength(result,3);
    PCardinal(pointer(result))^ := JSON_SQLDATE_MAGIC;
    result := result+PIso8601(@TimeStamp)^.Text(true);
  end;
end;
...
var L: integer;
begin
  result := Trim2(FindIniEntry(Content,'',Name+' ')); // 'Name = Value' format
...
begin // see http://www.garykessler.net/library/file_sigs.html for magic numbers
  ...
  if (Result='') and (FileName<>'') then begin
    case GetFileNameExtIndex(FileName,
       'png,gif,tiff,tif,jpg,jpeg,bmp,doc,docx,html,htm,css,js,ico') of
      0:   Result := 'image/png';
      1:   Result := 'image/gif';
      ...
      9,10:Result := 'text/html';
      11:  Result := 'text/css';
      12:  Result := 'application/x-javascript';
      13:  Result := 'image/x-icon';
      else begin
...
begin
  ...
  fillchar2(TempClass,sizeof(TempClass),0);
...
begin
  ...
  if cardinal(Index)<cardinal(n) then begin
    ...
    if ElemType<>nil then
      fillchar2(P[0],ElemSize,0); // avoid GPF
  end else
...
begin
  ...
  if fCountP<>nil then begin
    ...
    if cardinal(aIndex)>=PCardinal(PtrUInt(Value^)-sizeof(Integer))^ then
      exit;
...
begin // this method is faster than default System.DynArraySetLength() function
  ...
  if (p=nil) or (p^.refCnt=1) then begin
    ...
    if ElemType<>nil then begin
      ...
      fillchar2(pp^,minLength*elemSize,0);
  ...
  fillchar2(pa[OldLength],neededSize-OldLength-Sizeof(TDynArrayRec),0);
  Value^ := p;
end;
...
begin
  ...
  fillchar2(Elem,ElemSize,0); // always fill with zero binary content
end;
...
begin
  if (Source<>nil) and (ElemType=nil) then
    ...
    fillchar2(pointer(result)^,ElemSize,0);
...
begin
  ...
  if not NewValues then
    fillchar2(Values[0],n*sizeof(PUTF8Char),0); // SetLength() could leave it not void
  dec(n);
  if P=nil then
    exit;
  while P^<>'{' do
...
begin
  ...
  with TSynTableFieldProperties(fField.List[i]) do begin
    ...
    fillchar2(pointer(fDefaultFieldData)^,fDefaultFieldLength,0);
  end;
  SetLength(fDefaultRecordData,fDefaultRecordLength);
  fillchar2(pointer(fDefaultRecordData)^,fDefaultRecordLength,0);
end;
...
begin // this code should copy parameters without any reference count handling
  fillchar2(Value,sizeof(Value),0); // TVarData is expected to be bulk stack
...
begin
  ...
  with TSynTableFieldProperties(fField.List[F]) do
    if F in AvailableFields then begin
      ...
      fillchar2(Dest^,fDefaultFieldLength,0);
...
begin
  ...
  if IdemPChar(P,'COUNT(*) FROM ') then begin
    ...
    withID := false;
    fillchar2(Fields,sizeof(Fields),0);
  ...
  if IdemPropName(Prop,'WHERE') then begin
    ...
    if (PInteger(P)^ and $DFDFDFDF=NULL_DF) and (P[4] in [#0..' ',';']) then
      // NULL statement
...
begin
  ...
  {$ifdef UNICODE}fillchar2(Filler,sizeof(Filler),0);{$endif}
end;
...
initialization
  ...
  if not SupportsSSE2 then // back to default X87 code for older CPUs
    PatchCode(@fillchar2,@FillCharX87,FILLCHAR_SIZE);
      {$endif DELPHI5OROLDER}
      {$ifndef ISDELPHI2007ANDUP} // use faster FillChar/Move for older Delphi
  RedirectCode(GetAddressFromCall(@FillCharInvoke),@fillchar2);
