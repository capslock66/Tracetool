// ***************************************************************
//  madCrypt.pas              version: 1.3.0  ·  date: 2012-04-03
//  -------------------------------------------------------------
//  blowfish encryption
//  -------------------------------------------------------------
...
// 2012-04-03 1.3.0 added x64 and unicode support
...
procedure Encrypt (var data: AnsiString;    const password: AnsiString; iv: int64 = 0); overload; {$ifdef UnicodeOverloads}
procedure Encrypt (var data: UnicodeString; const password: AnsiString; iv: int64 = 0); overload; {$endif}
procedure Decrypt (var data: AnsiString;    const password: AnsiString; iv: int64 = 0); overload; {$ifdef UnicodeOverloads}
procedure Decrypt (var data: UnicodeString; const password: AnsiString; iv: int64 = 0); overload; {$endif}

// ***************************************************************

// this is the old version, which worked but was not 100% blowfish compatible
procedure OldEncrypt (buf: pointer; len: dword; const password: AnsiString); overload;
procedure OldDecrypt (buf: pointer; len: dword; const password: AnsiString); overload;
procedure OldEncrypt (var data: AnsiString; const password: AnsiString); overload;
procedure OldDecrypt (var data: AnsiString; const password: AnsiString); overload;

// ***************************************************************

// encode/decode a buffer (base64)
function Encode  (data: pointer; len: integer) : AnsiString;    overload;
function Decode  (data: pointer; len: integer) : AnsiString;    overload;
function DecodeW (data: pointer; len: integer) : UnicodeString; overload;

// encode/decode a string (base64)
function Encode  (const data: AnsiString   ) : AnsiString;    overload; {$ifdef UnicodeOverloads}
function Encode  (const data: UnicodeString) : AnsiString;    overload; {$endif}
function Decode  (const data: AnsiString   ) : AnsiString;    overload;
function DecodeW (const data: AnsiString   ) : UnicodeString; overload;

// ***************************************************************

var
  EncryptA : procedure (var   data: AnsiString; const password: AnsiString; iv: int64 = 0) = Encrypt;
  DecryptA : procedure (var   data: AnsiString; const password: AnsiString; iv: int64 = 0) = Decrypt;
  EncodeA  : function  (const data: AnsiString) : AnsiString = Encode;
  DecodeA  : function  (const data: AnsiString) : AnsiString = Decode;

// ***************************************************************

{$ifdef UnicodeOverloads}
  var
    EncryptW : procedure (var   data: UnicodeString; const password: AnsiString; iv: int64 = 0) = Encrypt;
    DecryptW : procedure (var   data: UnicodeString; const password: AnsiString; iv: int64 = 0) = Decrypt;
    EncodeW  : function  (const data: UnicodeString) : AnsiString = Encode;
{$else}
  procedure EncryptW (var   data: UnicodeString; const password: AnsiString; iv: int64 = 0);
  procedure DecryptW (var   data: UnicodeString; const password: AnsiString; iv: int64 = 0);
  function  EncodeW  (const data: UnicodeString) : AnsiString;
{$endif}
  ...
  for i1 := 0 to integer(len shr 3) - 1 do begin
    if iv <> 0 then begin
      for c1 := 7 downto 0 do begin
        TPByte(NativeUInt(@iv) + c1)^ := (TPByte(NativeUInt(@iv) + c1)^ + 1) and $ff;
        if TPByte(NativeUInt(@iv) + c1)^ <> 0 then
          break;
      end;
      c1 := SwapDword(TPCardinal(           @iv     )^);
      c2 := SwapDword(TPCardinal(NativeUInt(@iv) + 4)^);
...
{$ifdef UnicodeOverloads}
procedure Encrypt(var data: UnicodeString; const password: AnsiString; iv: int64 = 0);
{$else}
procedure EncryptW(var data: UnicodeString; const password: AnsiString; iv: int64 = 0);
{$endif}
var i1, i2 : integer;
begin
  if data <> '' then begin
    ...
    SetLength(data, i1 + i2);
    for i1 := i1 + 1 to i1 + i2 do
      data[i1] := WideChar(i2 shl 8 + i2);
    cryptBuf(pointer(data), Length(data) * 2, true, password, iv);
  end;
end;
...
{$ifdef UnicodeOverloads}
procedure Decrypt(var data: UnicodeString; const password: AnsiString; iv: int64 = 0);
{$else}
procedure DecryptW(var data: UnicodeString; const password: AnsiString; iv: int64 = 0);
{$endif}
var i1, i2 : integer;
...
begin
  ...
  if i1 > 0 then begin
    {$ifdef d6}
      UniqueString(data);
    {$endif}
    cryptBuf(pointer(data), i1 * 2, iv <> 0, password, iv);
    if (byte(data[i1]) <= 8) and (byte(data[i1]) > 0) and (word(data[i1]) = byte(data[i1]) shl 8 + byte(data[i1])) then begin
      b1 := true;
      for i2 := 1 to byte(data[i1]) - 1 do
        if data[i1 - i2] <> data[i1] then begin
          b1 := false;
...
procedure OldEncrypt(buf: pointer; len: dword; const password: AnsiString);
begin
  cryptBuf(buf, len, true, password, 0, true);
end;

procedure OldDecrypt(buf: pointer; len: dword; const password: AnsiString);
begin
  cryptBuf(buf, len, false, password, 0, true);
end;

procedure OldEncrypt(var data: AnsiString; const password: AnsiString);
begin
  UniqueString(data);
  cryptBuf(pointer(data), Length(data), true, password, 0, true);
end;

procedure OldDecrypt(var data: AnsiString; const password: AnsiString);
begin
  UniqueString(data);
...
function Encode(const data: AnsiString) : AnsiString; overload;
begin
  result := Encode(pointer(data), Length(data));
end;

{$ifdef UnicodeOverloads}
function Encode(const data: UnicodeString) : AnsiString; overload;
{$else}
function EncodeW(const data: UnicodeString) : AnsiString; overload;
{$endif}
begin
  result := Encode(pointer(data), Length(data) * 2);
end;
...
begin
  as1 := Decode(data, len);
  SetLength(result, Length(as1) div 2);
  if result <> '' then
    Move(as1[1], result[1], Length(result) * 2);
end;

function Decode(const data: AnsiString) : AnsiString; overload;
begin
  result := Decode(pointer(data), Length(data));
end;

function DecodeW(const data: AnsiString) : UnicodeString; overload;
var as1 : AnsiString;
begin
  as1 := Decode(pointer(data), Length(data));
  SetLength(result, Length(as1) div 2);
  if result <> '' then
    Move(as1[1], result[1], Length(result) * 2);
end;
