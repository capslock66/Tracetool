...
extern "C"
{
  ...
  //---------------------------------------------------------------------------------------------------------------
   ...
   __declspec(dllexport) void __cdecl CheckPlugInFile (unsigned PlugId , char * FileName , 
      char * PlugName , char * strException ) 
   {
      ...
      try {
         ...
         Loader = safe_cast<CppPluginLoader^>(domain->CreateInstanceFromAndUnwrap(Path,"CppPluginLoader")) ;  // TraceTool.
      } catch ( Exception  ^ ex ) {
         Singleton::trace ("Cpp wrapper : CheckPlugInFile() : FileName : " + strFileName + "\n") ;
