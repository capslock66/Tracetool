...
const
   BUFSIZE = 130000 ;  // Load 130K at once
var
   f:file of byte;
   ...
   begin
      ...
      buf[readchars+1] := 0;
      PtrBeg := pAnsichar(@buf) ;        // buf : array of byte
      ...
      PtrEnd := GotoEndOfPreviousLine (PtrEnd,  pAnsiChar(@buf)) ;

      // search the begin of line
      PtrBeg := GotoBeginLine (PtrEnd,  PAnsiChar(@buf)) ;
      ...
      while true do begin
         ...
         EndBlock := BeginBlock + (PtrBeg - pchar(@buf)) -1 ;
         ...
         PtrEnd := GotoEndOfPreviousLine (PtrBeg,  PAnsiChar(@buf)) ;


         // check if no more lines
         if PtrEnd = PAnsiChar(@buf) then begin
         ...
         if PtrBeg = PAnsiChar(@buf) then begin
...
begin
   ...
   try
      ...
      NotCompleteNode := nil ;
      NbLinesRead := 0 ;
      nbBlockToRead := (File_Size div BUFSIZE) + 1 ;
      ...
      while EndBlock > TitleOffset do begin
         ...
         seek(f,BeginBlock) ;
         blockread (f,buf,EndBlock-BeginBlock,readchars);
...
begin
   ...
   try
      while true do begin    // loop until end of file
         ...
         blockread (f,buf,65000+2,readchars);
